...
FOR EACH ROW
DECLARE
    nomCompletFilm PERSONNE.nomComplet%TYPE;
    photoFilm PERSONNE.photo%TYPE;
    bioFilm PERSONNE.bio%TYPE;
    categorie PERSONNE.categoriePersonne%TYPE;
BEGIN
    SELECT categoriePersonne, nomComplet, photo, bio
    INTO categorie, nomCompletFilm, photoFilm, bioFilm
    FROM Personne
    WHERE idPersonne = :NEW.idPersonne;
    IF categorie != 'PersonneFilm' THEN
        RAISE_APPLICATION_ERROR(-20001, 'La personne doit être de type PersonneFilm.');
    END IF;
    IF nomCompletFilm IS NULL OR photoFilm IS NULL OR bioFilm IS NULL THEN
        RAISE_APPLICATION_ERROR(-20001, 'Toutes les valeurs doivent être présentes.');
    END IF;
END;
/
...
(
    ...
    v_idAbonnement Abonnement.idAbonnement%TYPE;
BEGIN
    INSERT INTO AdresseBase(ville, province, pays)
    VALUES (v_ville, v_province, v_pays)
    RETURNING idAdresse INTO v_idAdresse;

    INSERT INTO Personne(idPersonne, dateNaissance, nomComplet, photo, bio, categoriePersonne, idAdresse)
    VALUES (v_idPersonne, v_dateNaissance, v_nom_complet, v_photo, v_bio, 'PersonneFilm', v_idAdresse);
...
BEGIN
    ...
    IF v_count = 0 THEN
        INSERT INTO Realisateur(idPersonne) VALUES (v_idPersonne);
    END IF;
